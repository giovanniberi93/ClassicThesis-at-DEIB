...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{Images/background_and_tools/husky.png}
...
In a differential drive system, the movement of the robot is only based on two separately driven wheels, placed on either side of the robot, on the same axis (see Figure \ref{fig:diffDrive}). By recalling the definition of \ac{ICC}, we observe that, if the weels are correctly aligned, a differential drive robot always have a well-defined \ac{ICC} and the slippage of the wheels is not very accentuated. \\
At each instant in time, since the \ac{ICC} is well-defined, the left and right wheels follow a path that moves around \ac{ICC} at the same angular speed $\omega$, and thus:
\begin{equation}
	\begin{cases}
...
where $L$ is the distance between the center of the two wheels.
