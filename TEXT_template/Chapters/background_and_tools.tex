...
environment.}}
The \ac{GRAPE} project, described with further details in Chapter \ref{chap:grapeProject}, is about the design and realization of an unmanned ground vehicle with control and operative tasks in a vineyard environment, so in this chapter we deal with topics concerning software development in robotics, estimation of the state of a robot, autonomous navigation, and motion planning.
...
\ac{ROS} is actually a \textit{meta-operating system}, that is, it's not an operating system in the traditional sense (it requires to be run on top of an another operating system; currently, the only officially supported OS is Linux Ubuntu), but it provides a peer-to-peer network that processes can use to create and process data together. This network is implemented through TCP, and it's called \textit{Computation Graph}. In this section, we're going to describe \ac{ROS} with more detail, with particular emphasis on the different techniques that nodes can use to communicate among them. Keep in mind that, even if these techniques differ significantly, they all are strongly typed, \textit{i.e.}, in order to define a channel (with \textit{channel} we now mean one of the technique that we are going to describe; it's not the name of a specific communication tool) you also need to define the types of message that are going to be exchanged through it.\\ \ac{ROS} embeds a lot of useful message types (\textit{e.g.}, \textit{LaserScan.msg}, \textit{PoseWithCovarianceStamped.msg}), grouped by domain (\textit{e.g.}, \textit{Sensor\_msgs}, \textit{Geometry\_msgs}). However a simple message definition language is provided, and users are encouraged to define their own message types to make them as self-explanatory as possible. We provide now a list of the components of the Computation Graph, \textit{i.e.}, \ac{ROS} processing units, and possible tools at disposal to make them communicate.

\begin{description}
\item[ROS Master] Even if the Computation Graph is a peer-to-peer network, a central process, called  \textbf{\ac{ROS} Master}, is required to exist, to provide naming and registration services to all the user processes in it. Once the processes have located each other through the services offered by the Master, they can communicate peer-to-peer without involving the central entity.

\item[nodes] The processes in the Computation Graph are called \textbf{nodes}, and they are the atomic units of the computational graph. The \ac{ROS} \ac{API} are available in C++, Python and Lisp, but C++ is the most widely used. One of the aims of \ac{ROS} is to be modular at a fine-grained scale, so a complex task should be achieved through cooperation of several different nodes, each with quite narrow tasks, rather than one large node that include all the functionalities. Nodes can use different techniques for communication, depending whether the message is a part of data stream or it is a request message (\textit{i.e.}, a response message is expected) and, in this last case, on the (expected) duration and complexity of the computation of the response.
...
\begin{figure}
	...
	\caption{\textit{Three phases of the setup of communication of nodes through topics.}}
	\label{fig:topicRegistration}
\end{figure}


\item[topics] Topics implements a \textit{publish-subscribe} paradigm, and they are the easiest way that nodes can use to communicate with each other. They basically are named channels, characterized by the type of the messages that are sent through it. When a node \textit{publishes} a message on a certain topic, the message is read by all the nodes that previously \textit{subscribed} to that topic, interfacing with the Master. Note that:
\begin{itemize}
...
\end{itemize}
We can easily conclude that this method is suitable for passing streams of data (\textit{e.g.}, the handler of a \ac{LIDAR} streams its measurements over the network, or a node publishes the velocities commands for the wheels of a robot), but there is no notion of a \textit{response} to a message, so it's not suitable for \textit{request-response} communication. 
...
\end{itemize}
These properties make Services suitable for punctual (in opposition to data stream) inter-node communication, such as:
\begin{itemize}
	\item request for parameters values
	\item request for frame acquisition, to a node that handles an RGB camera
	\item request for map cleaning, to a node that handles robot navigation
\end{itemize}
...
A final consideration about the \ac{ROS} mechanism for recording, and playing back, data published on topics. This software module is called \textit{rosbag}, and takes advantage of the fact that \ac{ROS} is aware of all the messages exchanged through its messaging system. This tool gets very useful in a project like \ac{GRAPE}, where is not trivial at all to simulate or replicate in a laboratory environment the physical context of a vineyard, and of course meaningful data are required in order to correctly validate algorithms and tools.
...
\textit{tf} is a \ac{ROS} library, designed in the early phases of \ac{ROS} development, to keep track of reference frames in a robotic system. Our system takes great advantage of \textit{tf} usage to accomplish both navigation and manipulation tasks. The goal of \textit{tf} is:
...
The utility of such a component is straightforward, even in quite simple robotic systems. We now describe a situation we stumbled upon exactly in the development of the \ac{GRAPE} project, where of the utility of \textit{tf} is very easy to understand; however, the context of this example is described in Chapter \ref{chap:kinovaArmChapter}. In this example a \ac{LIDAR}, mounted on top of the final joint of a robotic arm, acquires data while the arm is moving in order to create a point cloud that is going to be processed later. To get a meaningful point cloud, it's mandatory to keep track of the movement of the \ac{LIDAR} with respect to a point with speed equal to zero (\textit{e.g.}, the base link of the arm, or the base link of the whole robot), and this gets even more difficult because of the multiple (6 in our specific case) joints of the arm; but this problem can be easily addressed by means of \textit{tf}.\\
Frames are very useful for two main tasks:
\begin{itemize}
	\item represent the configuration of the robot, by assigning frames to the significant physical elements of the robot (\textit{e.g.}, joints, sensors, wheels). Since \textit{tf} graph is a tree, a root frame of the robot should exists; in typical configurations, the root frame name is \textit{base\_link}, and it's placed in the geometrical center of the robot. All other frames associated to physical components of the robot belong to \textit{base\_link} subtree.
...
\begin{figure}
	\centering
	\includegraphics[width=1.1\textwidth]{Images/background_and_tools/tfGraph.png}
...
To better understand the complexity of the problem, let's analyze an extremely simple model: a robot with a single, freely rotating wheel. In this frame, assuming rotary encoders on the wheel, we can think about measuring directly the wheel speed and integrate these measurements to get the traveled distance, and measure the variation in the orientation of the wheel to get the position. Unfortunately, a lot of imperfections exist, that may lead to errors in the outcome, for example:
	\begin{itemize}
		\item the wheel can be non perfectly perpendicular to the ground
		\item the friction between the floor and the wheel might not be enough to avoid slippage
...
Moreover, the probability of slippage gets higher in systems with more than one wheel, because for a system with multiple wheels to move without slippage, a point must exists around which all the wheels can move along a circular path. This point is called Instantaneous Center of Curvature (see Figure \ref{fig:icc}), and can be easily identified by looking for the intersection of the axis of all wheels. If the wheels axis does not intersect in a single point, the system will not be able to move without slipping.
However, even if the odometry estimated from the wheels is not a good solution if used alone, it can be used as a starting point for other, more complex, methods. For this reason we are now going to describe how to estimate the odometry starting from the wheels encoders in our specific robot. This computation is different according to the \textbf{motion model} of the considered robot. The robot we used is the Husky platform (see Figure \ref{fig:husky}) from Clearpath Robotics. It moves with a  \textit{skid steering} kinematics, that is a derivative of \textit{differential drive} kinematics. Thus, we're going to describe these two motion model with more details.
...
In a differential drive system, the movement of the robot is only based on two separately driven wheels, placed on either side of the robot, on the same axis (see Figure \ref{fig:diffDrive}), and optionally a central, non-actuated caster wheel for stability. The two side wheels are not steerable, so the changes of direction are realized through application of different speed to the two wheels. For example, intuitively, if the wheels move at the same speed and in the same direction, the robot will move straight; if the wheels move at the same speed but different directions, the robot rotates in place.
 By recalling the definition of Instantaneous Center of Curvature, we observe that, if the wheels are correctly aligned, a differential drive robot always has a well-defined Instantaneous Center of Curvature and the slippage of the wheels is not very accentuated. \\


At each instant in time, since the Instantaneous Center of Curvature is well-defined, both the left and right wheel follow a path that moves around Instantaneous Center of Curvature at the same angular speed $\omega$, and thus:
...
where $L$ is the distance between the center of the two wheels, $v_r$ and $v_l$ are, respectively, the linear velocity of the right and left wheel, $R$ is the signed distance between the Instantaneous Center of Curvature and the midpoint of the wheels. Note that the only parameter constant through time is $L$, since it's a physical property of the robot structure, while all other parameters evolve during the movement. \\
...
\begin{equation}
	V(t)=\frac{(v_r+v_l)}{2}
\end{equation}
where $V(t)$ represents the overall speed of the robot, and assuming to know the functions $v_r(t)$ and $v_l(t)$, \textit{i.e.}, the linear speed of the wheel in time, we can calculate $(x,y,\theta)$ by integrating the speed of the robot over time, that is:
...
The Husky platform (see Figure \ref{fig:husky}) belongs to a different kinematic model, that relies on four (instead of two) actuated wheels. However, the motion model of the Husky is similar to differential drive because:
\begin{itemize}
...
This type of motion model is called \textit{skid steering}, and is often use in real-world applications (see Figure \ref{fig:skidSteeringB}) because of its simple and robust mechanical structure, which leaves more room in the vehicle for the mission equipment. In
addition, it has good mobility on a variety of terrains, which makes it suitable
for all-terrain missions. This motion model also present a variety of problems and weakness. For example, if you recall what we told about Instantaneous Center of Curvature in Section \ref{sec:odometry}, it's clear that the Instantaneous Center of Curvature of such a model will always be at the infinite, since wheels are organized in 2 parallel rows that cannot steer. Thus, skid steering robots can't turn without slipping of the wheels! 
From a theoretical point of view, the main consequence is a complexity in obtaining an accurate kinematics and dynamic model of \textit{skid steering} \parencite{skidSteeringDifficult}. In the context of \ac{GRAPE} project this leads to two major practical problems:
\begin{itemize}
	\item the slippage of the robot leads to higher power consumption of the electric engines with respect to a system with explicit steering \parencite{skidSteeringConsumption}. This is something to be taken into account in the sizing phase of the power system of the robot.
...
\begin{itemize}
	\item the mass center of the robot is located at the geometric center of the body frame.
...
the measurement space (\textit{e.g.}, measuring the desired quantity with optical sensors and ultrasonic sensors) the system becomes less vulnerable against interference.
\end{itemize}
...
After the experimental campaign in Casciana Terme, ROAMFREE turned out to be harder to configure for our specific problem, while Robot Localization had some specific documentation about its usage in the required context\footnote{\url{http://docs.ros.org/kinetic/api/robot\_localization/html/integrating\_gps.html}},
so we opted for this last one. Further details about our usage or Robot Localization are given in Chapter \ref{chap:localization}, while we are now giving a few hints about its general functioning.
...
\begin{figure}
	\centering
	\includegraphics[width=0.6\textwidth]{Images/background_and_tools/robotLocalizationLogo.png}
...
\begin{itemize}
	\item \textit{nav\_msgs/Odometry}: the odometry estimation output by another node can be used entirely as input; this is the case of the odometry estimated only using wheels encoders, and it also allows for cascaded localization nodes.
...
Robot Localization can be configured also to publish the \textit{tf} transformation associated to the estimated odometry. This is one of the usage of \textit{tf} described in Section \ref{sec:tf}.
