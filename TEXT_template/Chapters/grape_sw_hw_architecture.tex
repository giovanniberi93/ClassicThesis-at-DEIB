...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.1cm,
  ...
  \node[state]                            (B) [above right = 0.7cm and 4.0cm of A] {$AS_1$};
  \node[state]                            (C) [above left = 0.7cm and 4.0cm of B]  {$S_2$};
  \node[state, fill=cyan]         (D) [above right = 0.7cm and 4.0cm of C] {$AS_2$};
  \node[state]                            (E) [above left = 0.7cm and 4.0cm of D]  {$S_3$}; 
  \node[state]                            (F) [above right = 0.7cm and 4.0cm of E] {$AS_3$};
  \node[state]                           (G) [above left = 0.7cm and 4.0cm of F]  {$S_4$};
  \node[state, fill=cyan]        (H) [above right = 0.7cm and 4.0cm of G] {$AS_4$};
  \node[state]                           (I)   [above left = 0.7cm and 4.0cm of H] {$S_5$};
  
  \node (Z) [draw=Maroon, fit= (A) (C) (E) (G) (I), inner sep=1.00cm, 
            dashed, fill=blue!10, fill opacity=0.3] {};
  \node [xshift=2.40ex,yshift=-1.5ex, Maroon] at (Z.south) {Supervisor};
\path 
  ...
  (I) edge[bend right, sloped, anchor=center, below]    node{}(A);
\end{tikzpicture}
