...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.8cm,
	...
	\label{fig:scanMotionFSA}
	\tikzstyle{every state}=[fill=white,draw=black,text=black]	
	\tikzset{every edge/.style={draw=black,sloped, anchor=center}} 
...
\path 
	(A)	edge[below]				node{Pre-pick pose}(B)
	
	(B)	edge[draw=white,above]	node{Compute marker}(C)
		edge[below]				node{features}(C)
		
	(C)	edge[below]				node{Target reached}(D)
  		edge[upLoop, decorate] 	node{Visual Servoing}(C)

  	(D)	edge[below]				node{Open fingers}(Z)
  	
  	(Z) 	edge[draw=white, above]	node{Extract}(E)
	  	edge[below]				node{dispenser}(E)
	  	
	(E)	edge[above]				node{Nail deployment}(F)
		edge[above]				node{Plant deployment}(H)

	(F)	edge[draw=white, above]	node{Fixed frontal}(G)
		edge[below]				node{position}(G)
		
	(H)	edge[above]				node{Frontal position}(I)
	
	(G)	edge[downLoop,below]	node{}(G);
